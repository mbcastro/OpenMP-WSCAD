...
\section{Regiões Paralelas e o Modelo Fork-Join}
	...
	Esse capítulo irá abordar os conceitos básicos de programação paralela
	com OpenMP. Primeiramente será apresentado o modelo base do OpenMP
	(Modelo Fork-Join). Em seguida, será apresentada a primitiva básica para
	a criação de regiões paralelas no código (\texttt{omp parallel}). Por
	fim, o serão apresentadas as formas de compartilhamento e privatização
	de dados oferecidas pelo OpenMP (\texttt{private}, \texttt{firstprivate}
	e \texttt{shared}).
