...
\section{Introdução}
	...
	através de exemplos práticos, disponíveis publicamente sob licença GNU
	GPL v3 em \url{www.github.com/lapesd/teaching-openmp}, e analisados
...
\label{section: sincronizacao}
	...
	diretivas de sincronização disponíveis na API \openmp.
		...
		Para esses casos, a API \openmp oferece duas diretivas principais. A primeira delas,
		...
		operações permitidas está disponível na especificação da API
		\openmp em \url{http://openmp.org/wp/openmp-specifications}.
		
		É importante notar que, para o caso de uma seção crítica contendo
...
\label{sec:paralelismo tarefas}
	...
	criadas com a diretiva \texttt{omp taskwait} (linhas 33--34). Com as
	tarefas correspondem a chamadas recursivas, essas tarefas poderão
	criar novas tarefas.
