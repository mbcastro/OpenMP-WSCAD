...
\section{Introdução}
	...
	% Advento de Processadores Multicore
	...
	$50\%$~\cite{LARUS08}, com o aumento da sua frequência de operação.
	No entanto, a dissipação de potência no \textit{chip}, inerente ao
	crescente número de transistores, impôs um limite físico ao
	desempenho máximo possível a ser alcançado com essa abordagem.
	Então, para contornar esse problema, grande parte da indústria de
	processadores agora investe na produção de processadores com
	diversos núcleos de processamento (\textit{multicores}).

	De fato, processadores \textit{multicore} já são largamente
	utilizados para a Computação de Alto Desempenho \cite{Asanovic09}.
	Porém, observa-se nessas arquiteturas uma grande disparidade entre o
	alto crescimento do número de \textit{cores} por processador e o
	baixo número de conexões entre processadores e o restante dos
	dispositivos.  Essa crescente disparidade faz com que o tempo de
	acesso a um dado armazenado na memória seja muito maior que o tempo
	necessário para processá-lo, o que conduz ao conhecido
	``\textit{memory wall problem}'' \cite{McKee-MemWall:2004}. Esse
	problema é atenuado atualmente através do uso de uma hierarquia de
	memórias \textit{cache} entre os processadores e a memória
	principal, que explora o princípio da localidade e permite assim
	acesso mais rápido aos dados e, consequentemente, uma melhora no
	desempenho global.

	A tendência atual da construção de arquiteturas \textit{multicore} é
	de um crescimento contínuo do número de \textit{cores}, resultando
	em arquiteturas compostas por dezenas ou até mesmo milhares de
	cores. Essa tendência é confirmada pelo \textit{website}
	TOP500\footnote{\url{http://www.top500.org}}
	(Figura~\ref{fig:top500}), que provê um \textit{ranking} atualizado
	dos 500 supercomputadores mais poderosos do mundo. Nessas
	arquiteturas o problema de escalabilidade é ainda maior, pois
	centenas ou milhares de processos (ou \textit{threads}) em execução
	simultânea disputam acesso a diversos recursos compartilhados como
	por exemplo \textit{cores}, memória principal, memórias
	\textit{cache}, entre outros.
