...
\end{lstlisting}
		...
		entrada foi fixado em $1680 \times 1680$, o número de
	...
	\label{subsection: escalonamento de iteracoes}
		...
		X7) para uma matriz de tamanho $1680 \times 1680$. O número de
		\textit{threads} $n$ foi variado de $2$ a $12$ e o tamanho de
		\textit{chunks} foi fixado em $1680/n$ e $1$, para as estratégias
		...
		resultados obtidos são apresentados na Figura \ref{fig: chunk
		size} e evidenciam que na estratégia \texttt{dynamic},
...
\end{lstlisting}
	...
	\label{subsection: reducao de operacoes}
		...
		resultado final para o problema sendo computado. Suporte
		eficiente a essa operação, denominada redução, é oferecida pelo
		OpenMP através da cláusula \texttt{reduction}. O Fragmento de
		Código (??) ilustra o uso da operação de redução para o cálculo
...
\section{Paralelismo de Tarefas e Diretivas OpenMP}
	...
	Isso é feito sob demanda seguindo um modelo clássico \textit{bag of tasks}. 
	...
	\todo[inline]{Explicar as diretivas \texttt{omp task} e \texttt{omp taskwait}. Explicar como deve
	ser usada a primitiva }
	\todo[inline]{Explicar a paralelização da implementação recursiva da soma da sequencia de Fibonacci.}
	
\begin{lstlisting}[frame=single, caption=Exemplo de uma implementação recursiva da soma da sequencia de Fibonacci usando tarefas.,
