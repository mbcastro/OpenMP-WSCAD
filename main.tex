...
\end{lstlisting}
		...
		\todo[inline]{Usar diferentes markers para cada curva em todas as figuras de
		resultados.}
	...
	\label{subsection: escalonamento de iteracoes}
		...
		\todo[inline]{Adicionar referencia no parágrafo abaixo.}
		...
		X7) para uma matriz de tamanho $1.680 \times 1.680$. O número de
...
\end{lstlisting}
	...
	\todo[inline]{Na Figura~\ref{fig:static-dynamic-guided} é guided ou dynamic? O texto cita dynamic mas
	na legenda da figura está guided.}
...
\section{Paralelismo de Tarefas e Diretivas OpenMP}
	...
	cálculo $F_{n-1}$ e $F_{n-2}$ são criadas com o uso da diretiva \texttt{omp task} (linhas 25--26 e 28--29) assim
	como as dependências são criadas com a diretiva \texttt{omp taskwait} (linhas 31--32).
	
	Note que o Código~\ref{listing:fibonacci} inclui uma condição de parada específica para a criação de tarefas (linha 22),
	evitando-se explicitamente a criação de tarefas quando o valor de $n$ é inferior a um valor específico (20 nesse exemplo).
	Basicamente, essa condição garante uma carga \textit{mínima} para computação de uma tarefa. Nesse caso, a carga
	mínima de uma tarefa é referente à computação de $F_{19}$. Em outras palavras, a computação será realizada sequencialmente
	por cada \textit{worker thread} quando $n<20$. Essa condição de parada para criação de tarefas é muito importante, tendo em vista
	que a criação e gestão de tarefas no OpenMP possui um sobrecusto.

\begin{lstlisting}[frame=single, caption=Exemplo de uma implementação recursiva da soma da sequencia de Fibonacci
...
long recursive_fibonacci(int n)
{
	...
	if (n < 20) return (recursive_fibonacci(n-1) + recursive_fibonacci(n-2));
	else
	{
		#pragma omp task shared(f1)
		f1 = recursive_fibonacci(n-1);

		#pragma omp task shared(f2)
		f2 = recursive_fibonacci(n-2);
...
\end{lstlisting}
	...
	Para avaliar o sobrecusto e o ganho de desempenho obtido com a utilização dessa condição de parada para criação de
	tarefas foram realizados experimentos em um...
	
	\todo[inline]{Gerar resultados na idkonn!}
